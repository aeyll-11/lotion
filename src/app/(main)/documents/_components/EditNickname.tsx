...
export default function EditNickname() {
  ...
  return (
    <div
      ...
      className="hover:bg-default-200 rounded-sm px-2 cursor-pointer relative select-none transition-opacity duration-300"
    >
      {user.nickname}
      <Dialog open={isOpen} onClose={() => setIsOpen(false)}>
        <div
          style={{ top: modalPosition.top + 10, left: modalPosition.left }}
          className="absolute flex flex-col gap-1 border rounded p-2 border-default-300 shadow-md font-normal text-sm min-w-64">
          <Dialog.Title className="text-text-grey">
            Edit nickname
          </Dialog.Title>
          <Dialog.Actions>
            <input
